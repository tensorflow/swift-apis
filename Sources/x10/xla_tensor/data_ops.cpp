...
xla::XlaOp MirrorPadInDimensionsBackward(
    xla::XlaOp grad_output, absl::Span<const int64_t> input_size,
    absl::Span<const int64_t> padding, tensorflow::MirrorPadMode mode) {
...
xla::XlaOp BuildMirrorPadBackward(xla::XlaOp grad_output,
  ...
  for (int64_t dim = grad_output_shape.rank() - 1; dim >= 0; --dim) {
