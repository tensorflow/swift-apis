...
/// `TensorFlowScalar`.
// This simply wraps a `TF_DataType` and allows user code to handle `TF_DataType` without importing
// CTensorFlow, which pollutes the namespace with TensorFlow C API declarations.
@_fixed_layout
...
public protocol _TensorFlowDataTypeCompatible {
    ...
    // Hooks used by the TFPartition pass for primitive operations on tensors. These should not be
    // called directly or implemented.

    /// This converts a TensorHandle that is known to have a 0d value into the scalar that it
    /// produces. Users should call the _TFGetScalarOrDie wrapper function.
    static func _getScalarOrDie(_ handle: TensorHandle<Self>) -> Self

    /// This converts a TensorHandle into a scalar if it is 0d, or returns nil otherwise. Users
    /// should call the _TFGetScalar wrapper function.
    static func _getScalar(_ handle: TensorHandle<Self>) -> Self?

    /// This indicates that it is safe to hoist the specified computation that creates a tensor to 
    /// being a parameter that is passed in from outside of the tensor program.
    static func _hoistableClosure(_ fn: () -> TensorHandle<Self>) -> TensorHandle<Self>
}
...
// This is the implementation of the _getScalarOrDie requirement for each concrete type below. We 
// use this round-about approach to implement the global _TFGetScalarOrDie function in order to 
// ensure that the noinline SIL functions below have non-generic type signatures. This is important 
// for the inner workings of the partitioning pass.
private func _TFGetScalarOrDieImpl<Scalar>(_ handle: TensorHandle<Scalar>) -> Scalar {
    return handle.makeHostCopy().scalar!
}

// This is the implementation of the _getScalar requirement for each concrete type below. We use 
// this round-about approach to implement the global _TFGetScalar function in order to ensure that 
// the noinline SIL functions below have non-generic type signatures.  This is important for the 
// inner workings of the partitioning pass.
...
extension Bool: TensorFlowScalar {
    ...
    public static func _hoistableClosure(_ fn: () -> TensorHandle<Bool>) -> TensorHandle<Bool> {
        return fn()
    }
}
...
extension Int8: TensorFlowScalar {
    ...
    public static func _hoistableClosure(_ fn: () -> TensorHandle<Int8>) -> TensorHandle<Int8> {
        return fn()
    }
}
...
extension UInt8: TensorFlowScalar {
    ...
    public static func _hoistableClosure(_ fn: () -> TensorHandle<UInt8>) -> TensorHandle<UInt8> {
        return fn()
    }
}
...
extension Int16: TensorFlowScalar {
    ...
    public static func _hoistableClosure(_ fn: () -> TensorHandle<Int16>) -> TensorHandle<Int16> {
        return fn()
    }
}
...
extension UInt16: TensorFlowScalar {
    ...
    public static func _hoistableClosure(_ fn: () -> TensorHandle<UInt16>) -> TensorHandle<UInt16> {
        return fn()
    }
}
...
extension Int32: TensorFlowScalar {
    ...
    public static func _hoistableClosure(_ fn: () -> TensorHandle<Int32>) -> TensorHandle<Int32> {
        return fn()
    }
}
...
extension UInt32: TensorFlowScalar {
    ...
    public static func _hoistableClosure(_ fn: () -> TensorHandle<UInt32>) -> TensorHandle<UInt32> {
        return fn()
    }
}
...
extension Int64: TensorFlowScalar {
    ...
    public static func _hoistableClosure(_ fn: () -> TensorHandle<Int64>) -> TensorHandle<Int64> {
        return fn()
    }
}
...
extension UInt64: TensorFlowScalar {
    ...
    public static func _hoistableClosure(_ fn: () -> TensorHandle<UInt64>) -> TensorHandle<UInt64> {
        return fn()
    }
}
...
extension BFloat16: TensorFlowScalar {
    ...
    public static func _getScalarOrDie(_ handle: TensorHandle<BFloat16>) -> BFloat16 {
        return _TFGetScalarOrDieImpl(handle)
    }
...
extension Float: TensorFlowScalar {
    ...
    public static func _hoistableClosure(_ fn: () -> TensorHandle<Float>) -> TensorHandle<Float> {
        return fn()
    }
}
...
extension Double: TensorFlowScalar {
    ...
    public static func _hoistableClosure(_ fn: () -> TensorHandle<Double>) -> TensorHandle<Double> {
        return fn()
    }
}
...
extension String: _TensorFlowDataTypeCompatible {
    ...
    public static func _hoistableClosure(_ fn: () -> TensorHandle<String>) -> TensorHandle<String> {
        return fn()
    }
}
