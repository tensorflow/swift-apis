...
extension TensorHandle: TensorSendableReceivable {
    ...
    func sendToAccelerator(_ computation: _TensorComputation, _ tensorID: Int) {
        ...
        internalConsistencyCheck(status != nil)
        let context = _ExecutionContext.global
        TFE_EnqueueNamedTensorFromCtx(context.eagerContext, Int32(tensorID), _cTensorHandle, status)
    ...
    static func scalar(_ scalar: Scalar) -> TensorHandle<Scalar> {
        ...
        let cTensorHandle = _TFCCreateCTensorHandle(scalar, Scalar.tensorFlowDataType._cDataType)
...
extension ResourceHandle: TensorSendableReceivable {
    ...
    // TODO: Remove this dummy `Scalar` typealias, currently required in order to conform to
    // `TensorSendableReceivable`.
