...
public class LAMB<Model: Differentiable>: Optimizer
    where Model.TangentVector: VectorProtocol & PointwiseMultiplicative & ElementaryFunctions,
          Model.TangentVector.VectorSpaceScalar == Float {
    public typealias Model = Model
    /// The learning rate.
    ...
    public func update(_ model: inout Model, along direction: Model.TangentVector) {
        ...
        secondMoments = secondMoments * beta2
        secondMoments += direction .* direction * (1 - beta2)
        r1 = sqrt(pow(direction, 2).sum())
        let denominator = Model.TangentVector.sqrt(secondMoments) + epsilon
        adamStep = stepSize * firstMoments ./ denominator
        r2 = sqrt(pow(adamStep, 2).sum())
        if r1 == 0 or r2 == 0:
            q = 1
        else
            q = min(r1/r2, 10)
        learningRate = learningRate * q

        model.move(along: -learningRate * adamStep)
